...
public class UserService implements IUserService {
    ...
    @Override
    @Transactional
    public void updateUser(Long id, AssignmentBuildingDTO buildingDTO) {
        List<AssignmentBuildingEntity> results = assignmentBuildingRepository.findAll();
        results.forEach(it->{
            if(it.getBuildingId().getId().equals(id)){
                assignmentBuildingRepository.deleteById(it.getId());
            }
        });
        for (String it : buildingDTO.getStaffId()) {
            AssignmentBuildingEntity assignmentBuildingEntity = new AssignmentBuildingEntity();
            UserEntity userEntity = userRepository.findById(Long.parseLong(it)).get();
            BuildingEntity buildingEntity = buildingRepository.findById(id).get();
            assignmentBuildingEntity.setBuildingId(buildingEntity);
            assignmentBuildingEntity.setStaffId(userEntity);
    ...
    @Override
    @Transactional
    public void updateCustomer(Long id, AssignmentCustomerDTO customerDTO) {
        CustomerEntity customerEntity = customerRepository.findById(id).get();
        List<UserEntity> staffs = userRepository.findByIdIn(customerDTO.getStaffId());
